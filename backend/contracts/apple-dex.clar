...
(define-read-only (get-token-balance)
  (contract-call? .apples-r get-balance (as-contract tx-sender))
)
...
(define-private (provide-liquidity-first (stx-amount uint) (token-amount uint) (provider principal))
    (begin
      ...
      (try! (contract-call? .apples-r transfer token-amount tx-sender (as-contract tx-sender)))
      ...
      (as-contract (contract-call? .apples-lp mint stx-amount provider))
    )
)
...
(define-public (provide-liquidity-additional (stx-amount uint))
  (let (
      ...
      (liquidity-token-supply (contract-call? .apples-lp get-total-supply))
    ...
    (begin 
      ...
      (try! (contract-call? .apples-r transfer tokens-to-transfer tx-sender contract-address))
      ;; mint LP tokens to tx-sender
      ;; inside as-contract the tx-sender is the exchange contract, so we use tx-sender passed into the function
      (as-contract (contract-call? .apples-lp mint liquidity-to-mint provider))
    )
  )
)
  ...
  (begin
    (asserts! (> liquidity-burned u0) err-zero-tokens)
      ...
      (begin 
        ...
        (try! (contract-call? .apples-lp burn liquidity-burned))
        ...
        (as-contract (contract-call? .apples-r transfer tokens-withdrawn contract-address burner))
      )
    )
  )
)
  ...
  (begin 
    ...
    (let (
      ...
      (begin
        ...
        (as-contract (contract-call? .apples-r transfer tokens-to-pay contract-address user-address))
      )
    )
  )
)
  ...
  (begin 
    ...
    (let (
      ...
      (begin
        ...
        (try! (contract-call? .apples-r transfer token-amount user-address contract-address))
        ;; transfer tokens from contract to user
